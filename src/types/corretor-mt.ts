export interface MTCorretor {
  id: string;
  tenant_id: string;
  franchise_id: string | null;
  user_id: string | null;
  legacy_id: number | null;
  nome: string;
  email: string | null;
  telefone: string | null;
  celular: string | null;
  foto_url: string | null;
  data_nascimento: string | null;
  endereco: string | null;
  numero: string | null;
  complemento: string | null;
  cep: string | null;
  location_bairro_id: string | null;
  location_cidade_id: string | null;
  location_estado_id: string | null;
  creci: string | null;
  creci_estado: string | null;
  creci_validade: string | null;
  especialidades: string[];
  comissao_percentual: number;
  total_vendas: number;
  total_alugueis: number;
  total_imoveis_ativos: number;
  valor_total_vendas: number;
  status: string;
  observacao: string | null;
  metadata: Record<string, unknown>;
  created_at: string;
  updated_at: string;
  deleted_at: string | null;
}

export interface MTCorretorCreate {
  tenant_id?: string;
  franchise_id?: string;
  user_id?: string;
  nome: string;
  email?: string;
  telefone?: string;
  celular?: string;
  foto_url?: string;
  data_nascimento?: string;
  creci?: string;
  creci_estado?: string;
  creci_validade?: string;
  especialidades?: string[];
  comissao_percentual?: number;
}

export interface MTCorretorUpdate extends Partial<MTCorretorCreate> {
  id: string;
}

export interface MTCorretorImovel {
  id: string;
  tenant_id: string;
  corretor_id: string;
  property_id: string;
  status: string;
  atribuido_em: string;
  finalizado_em: string | null;
  atribuido_por: string | null;
}
