...
const schema = z.object({ nome: z.string().min(2), creci: z.string().optional(), cpf: z.string().optional(), email: z.string().email().optional().or(z.literal("")), telefone: z.string().optional(), comissao_percentual: z.coerce.number().min(0).max(100).optional(), observacoes: z.string().optional() });
...
export default function CorretorEdit() {
  ...
  const { data: item, isLoading } = useQuery({ queryKey: ["mt-corretor", id], queryFn: async () => { const { data, error } = await supabase.from("mt_corretores" as any).select("*").eq("id", id!).single(); if (error) throw error; return data as any; }, enabled: isEditing });
  useEffect(() => { if (item) form.reset(item); }, [item, form]);
  const saveMut = useMutation({
    mutationFn: async (v: FV) => { const p = { ...v, tenant_id: tenant?.id, franchise_id: franchise?.id, updated_at: new Date().toISOString() }; if (isEditing) { const { data, error } = await supabase.from("mt_corretores" as any).update(p).eq("id", id!).select().single(); if (error) throw error; return data; } const { data, error } = await supabase.from("mt_corretores" as any).insert(p).select().single(); if (error) throw error; return data; },
    onSuccess: (d: any) => { qc.invalidateQueries({ queryKey: ["mt-corretores"] }); toast.success(isEditing ? "Atualizado" : "Criado"); navigate(`/corretores/${d.id}`); },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4"><Button variant="ghost" size="icon" onClick={() => navigate(-1)}><ArrowLeft className="h-4 w-4" /></Button><h1 className="text-2xl font-bold">{isEditing ? "Editar Corretor" : "Novo Corretor"}</h1></div>
