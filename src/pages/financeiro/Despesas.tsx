...
export default function Despesas() {
  ...
  // Filtrar apenas recorrentes tipo despesa
  const despesaRecurrings = useMemo(() => recurrings.filter(r => r.tipo === 'despesa'), [recurrings]);
  const activeRecurringsCount = useMemo(() => despesaRecurrings.filter(r => r.is_active).length, [despesaRecurrings]);

  const despesaCategories = useMemo(() => categories.filter(c => c.tipo === 'despesa'), [categories]);
  ...
  const totalMensalEstimado = useMemo(() => {
    return despesaRecurrings.filter(r => r.is_active).reduce((s, r) => s + r.valor, 0);
  }, [despesaRecurrings]);
        ...
        <div className="flex gap-2">
          {activeTab === 'recorrentes' && (
            <>
              ...
              <Button onClick={() => navigate('/financeiro/recorrentes/novo?tipo=despesa')}>
                <Plus className="h-4 w-4 mr-2" /> Nova Recorrente
              </Button>
            </>
          )}
          {activeTab === 'lancamentos' && (
            <Button onClick={() => navigate('/financeiro/lancamentos/novo?tipo=despesa')}>
          ...
          <Card>
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <div className="p-8 text-center text-muted-foreground">Nenhuma despesa encontrada</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>Esta acao ira cancelar a despesa "{tx.descricao}".</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Voltar</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => deleteTransaction(tx.id)}>Cancelar despesa</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
                  ...
                  <TableFooter>
                    <TableRow>
                      <TableCell colSpan={3} className="font-medium">Total Despesas</TableCell>
                      <TableCell className="text-right">
                        <div className="text-red-600 font-bold">-{formatCurrency(totalDespesas)}</div>
                      </TableCell>
                      <TableCell colSpan={2} />
                    </TableRow>
                  </TableFooter>
        ...
        <TabsContent value="recorrentes" className="space-y-4 mt-4">
          <Card>
            <CardContent className="p-0">
              {isLoadingRecurrings ? (
                <div className="p-8 text-center text-muted-foreground">Carregando...</div>
              ) : despesaRecurrings.length === 0 ? (
                <div className="p-8 text-center text-muted-foreground">
                  Nenhuma despesa fixa/recorrente cadastrada
                </div>
              ) : (
                <Table>
                  <TableHeader>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    "{rec.descricao}" não gerará mais lançamentos. Lançamentos pendentes serão cancelados. Lançamentos já pagos não serão afetados.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
          ...
          {despesaRecurrings.length > 0 && (
            <div className="text-sm text-muted-foreground">
              ...
              <span className="font-medium text-red-600">{formatCurrency(totalMensalEstimado)}</span>
              {' '}({despesaRecurrings.filter(r => r.is_active).length} despesas fixas ativas)
            </div>
          )}
        </TabsContent>
      </Tabs>
