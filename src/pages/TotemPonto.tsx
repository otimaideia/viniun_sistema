...
export default function TotemPonto() {
  ...
  const [nextPunchLabel, setNextPunchLabel] = useState('Entrada Manha');
  ...
  // Live clock — só atualiza nas telas onde é visível (cpf_input, success)
  const [currentTime, setCurrentTime] = useState(new Date());
  useEffect(() => {
    if (step !== 'cpf_input' && step !== 'success') return;
    setCurrentTime(new Date()); // atualiza imediatamente ao entrar na tela
  ...
  const handleConfirmCPF = useCallback(async () => {
    ...
    try {
      ...
      setNextPunchLabel(result.nextPunchLabel || 'Entrada');
  ...
  const handleRegister = useCallback(async () => {
    ...
    try {
      // Geo e registro em PARALELO — geo não bloqueia mais o registro
      const geoPromise = requestPosition().catch(() => null);

      // Registrar imediatamente (sem esperar geo)
      ...
      // Se geo chegou, atualizar o registro (fire-and-forget)
  ...
  const handleFullReset = useCallback(() => {
    ...
    setNextPunchLabel('Entrada Manha');
    setTotalPunches(0);
  ...
  // Determine action type from multi-punch state
  ...
  return (
    <TotemLayout>
      {/* Header */}
      <TotemHeader
        nomeUnidade={franchise?.nome_fantasia || 'Registro de Ponto'}
      ...
      <div className="text-center mb-3">
        <p className="text-white/90 text-sm font-medium">Registro de Ponto</p>
          ...
          <Card className="bg-white/95 backdrop-blur shadow-xl">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-sm text-gray-500">
                Tire sua foto para registrar: <span className="font-semibold text-[#662E8E]">{nextPunchLabel}</span>
              </p>
                  ...
                  {todayEntries.map((e, idx) => (
                    <div key={e.id} className="flex items-center justify-center gap-3 text-xs text-gray-500">
                      ...
                      {e.checkout_em ? (
                        ...
                        <span className="text-orange-500 font-medium">Saida pendente</span>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <Card className="bg-white/95 backdrop-blur shadow-xl">
            <CardContent className="p-4">
              ...
              <p className="text-center text-xs text-gray-400 mb-2">
                Proxima batida: <span className="font-semibold">{nextPunchLabel}</span>
                {totalPunches > 0 && ` (${totalPunches} batida${totalPunches > 1 ? 's' : ''} hoje)`}
              </p>
      ...
      {step === 'registrando' && (
        <Card className="bg-white/95 backdrop-blur shadow-xl">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-lg font-semibold text-gray-700">Registrando ponto...</p>
        ...
        <Card className="bg-white shadow-xl overflow-hidden">
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-2">
              {successMessage || 'Ponto registrado!'}
            </h2>
      ...
      {step === 'error' && (
        <Card className="bg-white shadow-xl overflow-hidden">
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Erro ao registrar</h2>
